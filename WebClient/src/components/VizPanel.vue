...
export default {
  ...
  data: () => ({
    ...
    news: [
      { id: 0, title: "Version 3.0", date: "2025-10-07", text: "Die neue API basiert auf dem <a href=\"https://notes.jan-oliver-ruediger.de/software/corpusexplorer-overview/\">CorpusExplorer</a>. Sie ist effizienter (schnellere Antwortzeiten / ...)", icon: "mdi-xml" },
      { id: 1, title: "Daten-Crash", date: "2025-09-05", text: "Aufgrund einer Fehlfunktion und der parallelen Migration auf einen neuen Server sind die Daten im Zeitraum 03.07.-04.09.2025 leider verloren gegangen. Bitte berücksichtigen Sie di...", icon: "mdi-alert-decagram-outline" },
      { id: 2, title: "Print-Publikation", date: "2024-06-27", text: "Der Artikel \"Tagesaktuelle Aufbereitung, Analyse und Exploration sprachlicher Daten aus RSS-Feeds\" steht OpenAccess unter folgender DOI zur Verfügung: <a href=\"https://doi.org/...\">[Link]</a>", icon: "mdi-book-open-variant" },
      { id: 3, title: "Poster-Präsentation", date: "2023-03-14", text: "Poster-Präsentation im Rahmen der DHd 2023 (Trier). <a href=\"https://doi.org/10.5281/zenodo.7715470\">[Link zum Abstract]</a>", icon: "mdi-human-male-board" },
      { id: 4, title: "Softwaredemo", date: "2022-07-14", text: "Softwaredemo …", icon: "mdi-video-vintage" },
      { id: 5, title: "Vortrag (GAL 2021)", date: "2021-09-15", text: "Die Vortragsfolien …", icon: "mdi-human-male-board" },
      { id: 6, title: "Version 2.0", date: "2021-06-18", text: "Die API-basierte Version 2.0 …", icon: "mdi-xml" },
    ],
    newsSelected: -1,
  }),
  ...
  mounted() {
      ...
      .then((response) => {
        ...
        fetch(baseUrl + "/norm", { method: "GET" })
          ...
          .then(() => {
            ...
            let data = "";
            if (queries.has("data")) data = queries.get("data");
            if (queries.has("locale")) {
              const locale = queries.get("locale");
    ...
    if (config.tutorialUrl.length < 1) {
      ...
      this.tutorial_iframe_src = config.tutorialUrl;
    }
  },
  ...
  computed: {
    totalLabel: function () {
      ...
      const totals = this.$store.state.owid.Total;
      const res = [];
